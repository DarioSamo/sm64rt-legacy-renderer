...
void mainCS(uint2 DTid : SV_DispatchThreadID)
{
    ...
    float4 samplesR = float4(samples[0].r, samples[1].r, samples[2].r, samples[3].r);
    float4 samplesG = float4(samples[0].g, samples[1].g, samples[2].g, samples[3].g);
    float4 samplesB = float4(samples[0].b, samples[1].b, samples[2].b, samples[3].b);
    float4 samplesA = float4(samples[0].a, samples[1].a, samples[2].a, samples[3].a);
